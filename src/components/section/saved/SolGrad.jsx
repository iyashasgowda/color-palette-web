...
const SolGrad = (props) => {
    const solgrad_ico = props.darkMode ? `${process.env.PUBLIC_URL}/assets/icons/light/solgrad.svg` : `${process.env.PUBLIC_URL}/assets/icons/dark/solgrad.svg`;
        ...
        if (gradient === null)
            ...
            solgrads = (
                <div className='solgrad-gradient-items'>
                    {gradient.map((item) => (
                        <Gradient key={item.key} data={item} shouldSave={false} shouldRemove={true}
                                  removeItem={removeGradient}></Gradient>
                    ))}
                </div>
            );
    }
        ...
        <div className='solgrad-container'>
            <div className='solgrad-header'>
                <div className='solgrad-header-title'>
                    <img src={solgrad_ico} alt='solgrad_ico'></img>
                    <p>{props.solgrad === 1 ? `Solid` : `Gradient`}</p>
                </div>

                <div className='solgrad-options'>
                    <select className='options' defaultValue={props.solgrad}
                            onChange={(e) => props.changeSolGrad(parseInt(e.target.value))}>
