      ...
      <div className='preset-container'>
         <div className='preset-header'>
            <div className='preset-header-title'>
               <img src={props.darkMode ? `${process.env.PUBLIC_URL}/assets/icons/light/preset.svg` : `${process.env.PUBLIC_URL}/assets/icons/dark/preset.svg`} alt='preset'></img>
               <p>{props.preset === 1 ? `Preset Gradient` : `Preset Quadrant`}</p>
            </div>

            <div className='preset-options'>
               <select className='options' defaultValue={props.preset} onChange={(e) => props.changePreset(parseInt(e.target.value))}>
         ...
         {props.preset === 1 && gradient !== null ? (
            <div className='preset-gradient-items'>
               {gradient.map((item, index) => (
                  <Gradient key={index} data={{ a_hex: item[0], b_hex: item[1], a_rgb: hex2rgbArray(item[0]), b_rgb: hex2rgbArray(item[1]) }} shouldSave={true} shouldRemove={false} />
               ))}
            </div>
         ) : props.preset === 2 && quadrant !== null ? (
            <div className='preset-quadrant-items'>
               {quadrant.map((item, index) => (
                  <Quadrant key={index} data={{ a_hex: item[0], b_hex: item[1], c_hex: item[2], d_hex: item[3], a_rgb: hex2rgbArray(item[0]), b_rgb: hex2rgbArray(item[1]), c_rgb: hex2rgbArray(item[2]), d_rgb: hex2rgbArray(item[3]) }} shouldSave={true} shouldRemove={false} />
               ))}
            </div>
         ) : (
            <></>
